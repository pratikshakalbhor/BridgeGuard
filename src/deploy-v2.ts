/**
 * Deploy BridgeGuard AI v2 contract to a Midnight network (undeployed by default; use --network preview|preprod for public networks).
 *
 * Same flow as src/deploy.ts but wired to the v2 contract
 * (contracts/managed/bridgeguard-v2) with isolated private-state ids so the
 * v1 deployment and CLI keep working untouched.
 */
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy BridgeGuard AI v2 to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
...
if (network === 'undeployed' && balance === 0n) {
  ...
  // funds the address from the network faucet. Mirrors src/deploy.ts.
  if (network !== 'undeployed' && networkConfig.faucet && balance === 0n) {
    ...
    while (true) {
      ...
      if (tn > 0n) {
        console.log(`\n  Funded! New tNIGHT balance: ${tn.toLocaleString()}\n`);
        balance = tn;
        break;
      }
  ...
  const dustState = await RxFirstSynced(walletCtx);
  ...
  if (dustState.dust.balance(new Date()) === 0n) {
    ...
    await RxFirstSyncedFiltered(walletCtx, (s) => s.dust.balance(new Date()) > 0n);
  }
  ...
  console.log('  Next: npx tsx src/server.ts\n');
}

// Rx helpers (avoid a hard dependency on rxjs operator piping in this script)
function RxFirstSynced(walletCtx: WalletContext) {
  return Rx.firstValueFrom(walletCtx.wallet.state().pipe(Rx.filter((s) => s.isSynced)));
}

function RxFirstSyncedFiltered(walletCtx: WalletContext, filter: (s: any) => boolean) {
  return Rx.firstValueFrom(
    walletCtx.wallet.state().pipe(Rx.throttleTime(5000), Rx.filter((s) => s.isSynced), Rx.filter(filter)),
  );
}
