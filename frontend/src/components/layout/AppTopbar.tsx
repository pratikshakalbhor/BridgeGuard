...
export function AppTopbar({ title, subtitle, live, onMenuClick }: AppTopbarProps) {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center justify-between gap-3 border-b border-slate-200 bg-white/70 px-4 backdrop-blur-xl dark:border-white/[0.08] dark:bg-midnight-950/70 sm:px-6">
      ...
      <div className="flex items-center gap-2.5">
        ...
        >
          ...
          {live ? 'Live chain' : 'Connecting…'}
        </span>
        <button
          type="button"
          onClick={toggleTheme}
          aria-label="Toggle theme"
          className="grid size-10 place-items-center rounded-xl border border-slate-200 text-slate-600 dark:border-white/10 dark:text-slate-300 transition-colors hover:border-cyan-400/50 hover:text-cyan-500"
        >
        ...
        {connecting ? (
          ...
          <>
            <span
              className={cn(
                'inline-flex items-center gap-1.5 rounded-xl border border-emerald-400/40 bg-emerald-400/10 px-3 py-2 text-xs font-semibold text-emerald-600 dark:text-emerald-300',
              )}
            >
            ...
            <button
              ...
              className="grid size-10 place-items-center rounded-xl border border-slate-200 text-slate-500 transition-colors hover:border-red-400/50 hover:text-red-500 dark:border-white/10 dark:text-slate-400"
            >
